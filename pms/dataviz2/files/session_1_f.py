...
colors_accent = ['#FF6B6B', '#4ECDC4', '#45B7D1', '#96CEB4', '#FFEAA7', '#DDA0DD', '#98D8C8', '#F7DC6F']
...
def exercise_1_time_series():
    ...
    for i, (country, code) in enumerate(zip(selected_countries, selected_codes)):
        ...
        ax.plot(country_data['Year'], country_data['GDP_Trillions'], 
                color=colors_primary[i], linewidth=2.5, linestyle=line_styles[i],
                label=country, marker='o', markersize=4, alpha=0.8)
    
    # Styling
    ax.set_xlabel('Year', fontweight='bold', fontsize=12)
    ax.set_ylabel('GDP (Trillions USD)', fontweight='bold', fontsize=12)
    ax.set_title('Nominal GDP Evolution of Major Economies\n(2000-2020)', 
    ...
    china_2020 = df_filtered[(df_filtered['Country Code'] == 'CHN') & 
                            (df_filtered['Year'] == 2020)]['GDP_Trillions'].iloc[0]
    ax.annotate(f'China: ${china_2020:.1f}T\nin 2020',
                xy=(2020, china_2020), xytext=(2015, china_2020 + 2),
...
def exercise_4_subplots():
    ...
    ax1.plot(world_data['Year'], world_data['GDP_Trillions'], 
             color='#2E86AB', linewidth=3, marker='o', markersize=3)
    ...
    df_period = df_countries[(df_countries['Year'] >= 2010) & 
    ...
    bars = ax3.bar(range(len(top_10_volatile)), top_10_volatile['std_billions'],
                   color=colors_viridis[:len(top_10_volatile)], alpha=0.8)
    ...
    # Main title
    fig.suptitle('Global Economy - Key Insights', 
